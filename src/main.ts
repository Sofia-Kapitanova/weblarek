...
api
  ...
  .then((products) => {
    console.log(
      ` Данные успешно получены с сервера! Найдено товаров: ${products.length}`,
    );
    ...
    if (products.length >= 2) {
      ...
      basketModel.add(products[0]);
      basketModel.add(products[1]);
      console.log(
      ...
      // Удаляем один товар
      basketModel.remove(products[0].id);
      console.log(
        ` После удаления товара ${products[0].id} (getItems()):`,
        basketModel.getItems(),
      );
...
// 0. Создаем общий брокер событий, необходимый для конструкторов
const events = new EventEmitter();

// Включаем логирование всех событий в консоль, чтобы видеть работу моделей через emit
events.onAll((event) => {
    console.log(`📡 [Событие брокера]: ${String(event.eventName)}`, event.data);
});
...
// Проверяем сохранение товаров из файла data.ts
productsModel.setProducts(apiProducts.items);
console.log('🛒 Массив всех товаров из каталога:', productsModel.getProducts());
...
if (firstProductId) {
    ...
    console.log(`🔍 Товар найденный по ID (${firstProductId}):`, singleProduct);
}

// Проверяем работу превью (выбора товара для модального окна)
productsModel.setPreviewProduct(apiProducts.items[0]);
...
if (testProduct1 && testProduct2) {
    ...
    console.log('🔢 Количество товаров в корзине (ожидаем 2):', basketModel.getCounter());
    console.log('💰 Общая стоимость товаров:', basketModel.getTotalPrice());
    console.log(`❓ Проверка наличия первого товара (ожидаем true):`, basketModel.checkItem(testProduct1.id));
    
    // Проверяем удаление одного товара
    basketModel.remove(testProduct1.id);
    console.log('🗑️ Корзина после удаления первого товара:', basketModel.getItems());
    console.log('🔢 Новый счетчик товаров (ожидаем 1):', basketModel.getCounter());
...
console.log('❌ Текущие ошибки валидации (ожидаем payment и phone):', buyerModel.validate());
...
console.log('✅ Ошибки валидации после полного заполнения (должен быть пустой объект):', finalErrors);
console.log('📝 Итоговые данные покупателя из модели:', buyerModel.getBuyerData());
